...
dd_build_oa_lkps_maps <- function(y, cx = NA, check_lsoa = FALSE, verbose = FALSE){
    ...
    if(check_lsoa){
        ...
        setnames(yc, yn, 'X')
        yc <- output_areas[, .(OA, LSOA)][yc, on = 'OA']
...
# x two columns only dataframe: one OA, the other the dissolving var
#' dd_dissolve_oa
#' 
#' Build a lookup table with output areas as source
#' 
...
#' @param s a set of integers between 5 and 95 to indicate the percentage(s) to simplify 
#' @param verbose indicate if each step should be commented
#'
#' @return a data.table with two columns
#'
...
dd_dissolve_oa <- function(y, wgs84 = TRUE, s = NA, verbose = FALSE){
    ...
    y <- lapply(unique(yb$RGN), \(z) yb |> filter(RGN == z) |> ms_dissolve(yn))
    if(verbose) message('* Binding back Regions...')
    y <- do.call('rbind', y) |> ms_dissolve(yn)
    if(wgs84){
...
# simplifying boundaries multiple percentages
dd_simplify_bnd <- function(x, nm, out_path = dmpkg.funs::bnduk_path, verbose = FALSE){
